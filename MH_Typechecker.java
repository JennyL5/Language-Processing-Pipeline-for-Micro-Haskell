...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable { 
    	...
    	if (exp == null) {
    		...
    		//e.g. 1234 string of numbers
    		String str = exp.value();
    		char [] c = str.toCharArray();
    		
    		for (char i : c) {
    			if (!CharTypes.isDigit(i)) {
    				throw new TypeError("incorrect number type");
    			}
    		}
    		return (IntegerType);
    		...
    		if (!exp.value().equals("True")  || !exp.value().equals("False")) {
    			throw new TypeError ("incorrect boolean type");
    		}
    		return (BoolType);
    		
    		
    	} else if (exp.isAPP()) { // for functions e.g. "e_1 e_2"
    		
    		if (!(computeType(exp.first(),env).isFun())) { //for a function
    			throw new TypeError ("not a function"); 
    			} else if ((computeType(exp.second(),env).equals(computeType(exp.first(),env).left()))) {
        			return computeType(exp.first(),env).right();
    			}
				throw new TypeError ("variable type do not match");

    		
    	} else if (exp.isINFIX()) {
    		MH_TYPE a = computeType(exp.first(), env);
    		MH_TYPE b = computeType(exp.second(), env);
    		String str = exp.infixOp();
    		
    		if (a.equals(IntegerType) && b.equals(IntegerType)) { //for integer
    			if (str.equals("+") || str.equals("-")) {
            		return IntegerType;
            	} else if (str.equals("==") || str.equals("<=")) {
            		return BoolType;
            	} else {
            		throw new TypeError("incorrect infix operator");
            	}
    		}
			else {throw new TypeError("both inputs must be integer types");
			}
    		
    		
    	} else if (exp.isIF()) {
    		MH_TYPE a = computeType(exp.first(),env); // condition
    		MH_TYPE b = computeType(exp.second(), env); // statement 1
    		MH_TYPE c = computeType(exp.third(),env); // statement 2
    		
    		if (!a.isBool()){
    			throw new TypeError("condition type is invalid");
    		} else if (!b.equals(c)) {
    			throw new TypeError ("comparing different variable types");
    		} else {
    			return b;
    		}
    	}
